...
&\Pr_{x \leftarrow U_n}(h(x')=h(x)) * \Pr_{x \leftarrow U_n}(x = x' | h(x) = h(x')) =\\
& \Pr_{x \leftarrow U_n}(h(x')=h(x)) * \frac{1}{2^n}
\end{align*}
...
&  \Pr_{x \leftarrow U_n}(h(x')=h(x)) \cdot (1-\frac{1}{2^n}) \geq\\
& \varepsilon (1-\frac{1}{2^n})
\end{align*}
